...
void selsnap(int *x, int *y, int direction) {
  ...
  case SNAP_LINE:
    ...
    *x = (direction < 0) ? 0 : term.col - 1;
    if (direction < 0) {
      for (; *y > 0; *y += direction) {
        if (!(TLINE(*y - 1)[term.col - 1].mode & ATTR_WRAP)) { break; }
      }
    } else if (direction > 0) {
      for (; *y < term.row - 1; *y += direction) {
        if (!(TLINE(*y)[term.col - 1].mode & ATTR_WRAP)) { break; }
      }
    }
    break;
  }
...
void tsetmode(int priv, int set, int *args, int narg) {
  ...
  for (lim = args + narg; args < lim; ++args) {
    if (priv) {
      switch (*args) {
      case 1: /* DECCKM -- Cursor key */ xsetmode(set, MODE_APPCURSOR); break;
      case 5: /* DECSCNM -- Reverse video */ xsetmode(set, MODE_REVERSE); break;
      case 6: /* DECOM -- Origin */
...
void strhandle(void) {
  ...
  case ']': /* OSC -- Operating System Command */
    ...
    case 104: /* color reset, here p = NULL */
      ...
      if (xsetcolorname(j, p)) {
        if (par == 104 && narg <= 1) return; /* color reset without parameter */
  ...
  case 'k': /* old title set compatibility */
    ...
    return;
  case 'P': /* DCS -- Device Control String */ term.mode |= ESC_DCS;
...
int tlinehistlen(int y) {
  ...
  if (TLINE_HIST(y)[i - 1].mode & ATTR_WRAP) return i;

  while (i > 0 && TLINE_HIST(y)[i - 1].u == ' ') --i;
...
void externalpipe(const Arg *arg) {
  ...
  for (n = 0; n <= HISTSIZE + 2; n++) {
    ...
    for (; bp < end; ++bp)
      if (xwrite(to[1], buf, utf8encode(bp->u, buf)) < 0) break;
    if ((newline = TLINE_HIST(n)[lastpos].mode & ATTR_WRAP)) continue;
    if (xwrite(to[1], "\n", 1) < 0) break;
    newline = 0;
  }
  if (newline) (void)xwrite(to[1], "\n", 1);
  close(to[1]);
  /* restore */
...
void tcontrolcode(uchar ascii) {
  ...
  case 0x88: /* HTS -- Horizontal tab stop */ term.tabs[term.c.x] = 1; break;
  case 0x89: /* TODO: HTJ */
  case 0x8a: /* TODO: VTS */
  case 0x8b: /* TODO: PLD */
  case 0x8c: /* TODO: PLU */
...
int eschandle(uchar ascii) {
  ...
  case ']': /* OSC -- Operating System Command */
  case 'k': /* old title set compatibility */ tstrsequence(ascii); return 0;
  ...
  case 'E':      /* NEL -- Next line */
    tnewline(1); /* always go to first col */
    break;
  case 'H': /* HTS -- Horizontal tab stop */ term.tabs[term.c.x] = 1; break;
  ...
  case '=': /* DECPAM -- Application keypad */
    ...
    break;
  case '>': /* DECPNM -- Normal keypad */ xsetmode(0, MODE_APPKEYPAD); break;

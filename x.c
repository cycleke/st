...
void bmotion(XEvent *e) {
  if (!xw.pointerisvisible) {
    ...
    if (!IS_SET(MODE_MOUSEMANY)) xsetpointermotion(0);
  }
...
void xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len,
  ...
  if (y == 0) xclear(winx, 0, winx + width, win.hborderpx);
  ...
  if (!b) { XftDrawGlyphFontSpec(xw.draw, fg, specs, len); }
...
void run(void) {
  ...
  for (timeout = -1, drawing = 0, lastblink = (struct timespec){0};;) {
    ...
    if (FD_ISSET(ttyfd, &rfd) || xev) {
      if (!drawing) {
        trigger = now;
        if (IS_SET(MODE_BLINK)) { win.mode ^= MODE_BLINK; }
        lastblink = now;
        drawing = 1;
      }
    ...
    timeout = -1;
    blinkcursor = win.cursor == 0 || win.cursor == 1 || win.cursor == 3 ||
                  win.cursor == 5;
...
int main(int argc, char *argv[]) {
  ...
  case 'A': opt_alpha = EARGF(usage()); break;
